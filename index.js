...
const displayEntries = () => {
  ...
  const rows = entries
    .map((entry) => {
      ...
      const row = `<tr>${name} ${email} ${password} ${dob} ${Accept Conditions}</tr>`;
      return row;
    })
    .join("\n");
...
form.addEventListener("submit", Save User From);
...
dateELE.addEventListener("change", () => {
  ...
  dateELE.style.border = "4px solid rgba(0, 0, 0, 0.4)";
  if (age < 18 || age > 55) {
    dateELE.setCustomValidity("Your age isn't between 18 and 55");
    dateELE.style.border = "4px solid blue";
    return;
  } else {
...
function validate(ele) {
  if (ele.validity.typeMismatch) {
    ele.setCustomValidity("******The Email is not in required format*****");
    ele.reportValidity();
  } else {
